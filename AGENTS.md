...
- 涉及程式行為、依賴或測試基礎設施時，主代理須在每個有意義的開發階段
  修改穩定、相關測試通過後，於階段提交前主動安排獨立審查，不等使用者
  提醒或 PR 上的其他 AI 留言。交付前確認任務差異及階段間的互動均已涵蓋。
- 使用未參與該階段實作、具獨立上下文的 reviewer，或另啟本機 code review
  工作階段，選擇其一即可。審查任務僅唯讀檢視與回報，不修改、不提交，
  也不再為審查任務安排另一輪 reviewer。純分析與單純回覆不觸發本流程；
  純文件、註解或格式變更不強制額外模型審查。
- 指派時提供任務目的、審查基準與版本、差異範圍。Reviewer 須閱讀本檔，
  追查相關未修改的使用端、契約、正常與失敗路徑及測試缺口；依變更檢查
  執行身分／有效設定、儲存與讀取一致性、程序與檔案清理等風險。
  不要求任務外清理、純個人風格修改或未被要求的向後相容層。
- Findings 須包含位置、觸發條件、預期與實際行為、影響及證據；區分已
  重現問題與尚待驗證的疑慮。主代理逐項核實，確認問題後修正並補回歸
  測試；不採納時提供具體理由。修正後複查修正及連帶影響，不以 AI 判斷
  取代驗證，也不以工具正常結束或沒有 findings 宣稱程式完全正確。
- 審查期間保持受查內容穩定；內容改變時，補查新差異與影響範圍後才能
  稱該版本已審查。相同內容且無未解疑慮時，不因 commit、push 或回覆而
  重跑。無法完成審查時回報阻礙及未涵蓋範圍，不宣稱已通過。
- 操作方式見 [開發指南](docs/development.md#開發階段的程式碼審查)。這是
  代理須主動執行的流程，不是 hook 強制門禁；不將模型呼叫加入
  `check-full.sh` 或 commit／push hooks。Code review、離線測試、PR 文件
  一致性審查與真實模型策略測試各有用途，不能互相替代。
...
- 說明開發階段審查的範圍、結果及 findings 處理；未執行時說明原因。
