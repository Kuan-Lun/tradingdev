...
if TYPE_CHECKING:
    ...
    from quant_backtest.data.schemas import WalkForwardConfig
...
class DataSplitter:
    ...
    def _explicit_split(
        ...
        train_mask = (
            ts >= _to_utc_ts(cfg.train_start)  # type: ignore[arg-type]
        ) & (
            ts
            < _to_utc_ts(cfg.train_end)  # type: ignore[arg-type]
            + pd.Timedelta(days=1)
        )
        test_mask = (
            ts >= _to_utc_ts(cfg.test_start)  # type: ignore[arg-type]
        ) & (
            ts
            < _to_utc_ts(cfg.test_end)  # type: ignore[arg-type]
            + pd.Timedelta(days=1)
        )
