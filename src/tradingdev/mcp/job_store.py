...
class JobRecord(BaseModel):
    """Typed filesystem job record for the temporary JSON-backed store."""
...
def _read_jobs_locked() -> dict[str, JobRecord]:
    _ensure_dirs()
        ...
        try:
            if JOBS_FILE.exists():
                ...
                if not isinstance(raw, dict):
                    return {}
                return {
                    str(job_id): JobRecord.model_validate(record)
...
def _write_jobs_locked(jobs: dict[str, JobRecord]) -> None:
    _ensure_dirs()
    tmp = JOBS_FILE.with_suffix(".tmp")
    with open(LOCK_FILE, "a") as lf:
        fcntl.flock(lf, fcntl.LOCK_EX)
        try:
            payload = {job_id: _record_to_dict(job) for job_id, job in jobs.items()}
            tmp.write_text(
...
def update_job(job_id: str, **fields: Any) -> None:
    ...
    if job_id not in jobs:
        ...
        return
    updated = _record_to_dict(jobs[job_id])
    updated.update(fields)
    jobs[job_id] = JobRecord.model_validate(updated)
    _write_jobs_locked(jobs)
...
def get_job(job_id: str) -> dict[str, Any] | None:
    ...
    jobs = _read_jobs_locked()
    return _record_to_dict(jobs[job_id]) if job_id in jobs else None
...
def list_all_jobs() -> list[dict[str, Any]]:
    ...
    jobs = _read_jobs_locked()
    return sorted(
        (_record_to_dict(job) for job in jobs.values()),
