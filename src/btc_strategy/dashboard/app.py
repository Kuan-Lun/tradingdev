...
Launch::
    ...
    streamlit run src/btc_strategy/dashboard/app.py \
        -- --config configs/xgboost_strategy.yaml

Requires a cached PipelineResult produced by::

    python -m btc_strategy.main --config configs/xgboost_strategy.yaml
"""
...
from btc_strategy.dashboard.analysis import (  # noqa: E402
    available_months,
    build_equity_series,
    build_trades_df,
...
from btc_strategy.data.schemas import BacktestConfig  # noqa: E402
from btc_strategy.utils.cache import load_cached_result  # noqa: E402
from btc_strategy.utils.config import load_config  # noqa: E402

# ---------------------------------------------------------------------------
# Streamlit page config
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="BTC/USDT Backtest Dashboard",
    layout="wide",
)
...
def _render_charts(  # noqa: PLR0915
    ...
    cols[5].metric(
        "Volume",
        f"{metrics['total_volume_usdt']:,.0f} USDT",
    )

    # --- Prepare data -----------------------------------------------------
    equity = build_equity_series(
        result.equity_curve, result.timestamps
    )
    trades_df = build_trades_df(
        result.trades, result.timestamps
    )
    ...
    if is_volume_mode:
        ...
        target_val = params_section.get(
            "monthly_volume_target_usdt"
        )
        if isinstance(target_val, (int, float)):
            monthly_target = float(target_val)

    # --- Tabs -------------------------------------------------------------
    tab_pnl, tab_hist, tab_streak, tab_monthly, tab_mdd = (
        st.tabs(
            [
                "Cumulative PnL",
    ...
    with tab_pnl:
        if unit == "USDT":
            pnl_series = cumulative_pnl(
                eq_view, result.init_cash
            )
            y_label = "Cumulative PnL (USDT)"
        else:
            pnl_series = cumulative_pnl_pct(
                eq_view, result.init_cash
            )
    ...
    with tab_hist:
        if tr_view.empty:
            ...
            if unit == "%" and tr_view["size_usdt"].sum() > 0:
                pnl_col = (
                    tr_view["net_pnl"]
                    / tr_view["size_usdt"]
                    * 100
...
def _render_simple(
    ...
    st.caption(
        f"Mode: **simple** | Config: `{config_path}`"
    )

    # --- Sidebar controls
    with st.sidebar:
        st.header("Controls")
        default_unit = "USDT" if is_volume_mode else "%"
        unit = st.radio(
            "Display unit",
            ["USDT", "%"],
            index=0 if default_unit == "USDT" else 1,
            horizontal=True,
        )
        months = ["全期間"] + available_months(
            result.timestamps
        )
...
def _render_walk_forward(
    ...
    st.title(f"Backtest Dashboard — {strategy_name}")
    st.caption(
        f"Mode: **walk-forward** | Config: `{config_path}`"
    )
    ...
    with st.sidebar:
        ...
        fold_labels = [
            f"Fold {f.fold_index}" for f in folds
        ]
        selected_fold_label = st.selectbox(
            "Fold", fold_labels
        )
        ...
        result: BacktestResult | None = (
            fold.test_backtest
            if is_test
            else fold.train_backtest
        )
        if result is None:
            st.error(
                "BacktestResult not available for this split."
            )
            st.stop()
            return
...
def main() -> None:
    ...
    if _PIPELINE_KEY not in st.session_state:
        st.session_state[_PIPELINE_KEY] = _load_pipeline(
            config_path
        )

    pipeline: PipelineResult = st.session_state[
        _PIPELINE_KEY
    ]
