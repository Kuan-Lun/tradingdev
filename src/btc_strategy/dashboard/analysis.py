...
def build_equity_series(
    ...
    if timestamps is not None:
        index: pd.DatetimeIndex | pd.RangeIndex = pd.DatetimeIndex(
            timestamps
        )
    else:
...
def rolling_mdd_absolute(
    ...
    rolling_mdd = drawdown.rolling(
        window=window_bars, min_periods=window_bars
    ).max()
    return rolling_mdd
...
def monthly_volume(
    ...
    Returns DataFrame with columns ``month`` (str) and ``volume_usdt``.
    """
    if trades_df.empty or "timestamp" not in trades_df.columns:
        return pd.DataFrame(columns=["month", "volume_usdt"])

    df = trades_df.copy()
    df["month"] = pd.to_datetime(df["timestamp"]).dt.to_period("M")
    grouped = (
        df.groupby("month")["size_usdt"]
        .sum()
        .multiply(2)  # round-trip volume
        .reset_index()
    )
    grouped.columns = pd.Index(["month", "volume_usdt"])
